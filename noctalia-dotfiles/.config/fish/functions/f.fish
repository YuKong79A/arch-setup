function f
    ...
    if test -n "$SELECTED_IMG"; and test -f "$SELECTED_IMG"
        # 显示图片
        fastfetch --logo "$SELECTED_IMG" --logo-width "$LOGO_WIDTH" --logo-height "$LOGO_HEIGHT" --logo-padding-right "$LOGO_PADDING_RIGHT" --logo-preserve-aspect-ratio true $ARGS_FOR_FASTFETCH
